...
[[ stitchable ]] half4 playerArtworkGlass(
    ...
    // Refraction falls to zero within an eight-point rim. Inward sampling
    // avoids pulling transparent pixels from outside the clipped cover.
    float rim = 1.0 - smoothstep(0.0, 8.0, depth);
    float bend = rim * rim * 4.2;
    float dispersion = 0.45 * rim * rim;
    ...
    float2 light = normalize(float2(-0.65, -1.0));
    float facing = dot(normal, light);
    float highlight = pow(max(facing, 0.0), 3.0);
    float counterlight = pow(max(-facing, 0.0), 5.0);
    float lip = exp(-pow((depth - 1.1) / 0.85, 2.0));
    float caustic = exp(-pow((depth - 4.0) / 1.5, 2.0));
    color *= half(1.0 - 0.16 * rim * (1.0 - highlight));
    float reflection = lip * (0.12 + 0.48 * highlight + 0.24 * counterlight)
        + caustic * counterlight * 0.15;

    // Broad, faint reflection across the face.
    float2 uv = position / max(size, float2(1.0));
    float sweep = uv.y + 0.45 * uv.x - 0.24;
    reflection += 0.065 * exp(-pow(sweep / 0.19, 2.0));
